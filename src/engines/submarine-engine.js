...
let submarineSettled = null; // null | 'before' | 'after'

// Perf fix, not in the frozen legacy/submarine-engine.legacy.js reference:
// this handler used to run its full body — several getBoundingClientRect()
// reads interleaved with style writes — synchronously on every native
// `scroll` event. Lenis (smooth-scroll.js) fires many scroll events per
// animation frame during inertia, so each one forced a synchronous layout
// recalculation (the browser has to resolve layout before it can answer a
// post-write getBoundingClientRect() call). That's exactly the stutter felt
// scrolling into/around #contact: the "prevent overlap with #contact title"
// block below adds two more forced-layout reads once the section is close.
// rAF-coalescing collapses however many scroll events land in a frame into
// one execution of the handler, capping the layout-thrash cost to once per
// paint instead of once per event — no change to what it computes or does.
let submarineScrollScheduled = false;
window.addEventListener('scroll', () => {
    if (submarineScrollScheduled) return;
    submarineScrollScheduled = true;
    requestAnimationFrame(() => {
        submarineScrollScheduled = false;
        handleSubmarineScroll();
    });
});

function handleSubmarineScroll() {
    ...
    // Perf fix, not in the frozen legacy reference: everything below this
    // point costs at least one forced-layout read (stickyElement.offsetHeight),
    // and the "prevent overlap with #contact title" block a few lines down
    // costs two more (a getBoundingClientRect() call right after a style
    // write forces the browser to resolve layout before it can answer).
    // None of that can produce a visible change once the whole 350vh track
    // is well clear of the viewport — which is most of the time spent
    // scrolling through Contact/Footer, or before ever reaching Delivery.
    // guardBand (2x viewport) comfortably covers the enter/exit fade zones
    // below (max 1.4x viewport) with margin, so nothing in the active range
    // is cut short — this only skips work once truly nothing is animating.
    const guardBand = window.innerHeight * 2;
    if (rect.bottom < -guardBand || rect.top > guardBand) {
        const wantState = rect.top > guardBand ? 'before' : 'after';
        if (submarineSettled !== wantState) {
            submarineSettled = wantState;
            if (wantState === 'after') {
            ...
            window.updateSubmarineState(wantState === 'after' ? 4 : 0);
        }
        return;
    }
    submarineSettled = null;
